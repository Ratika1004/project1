...
const msg = (key) => (_, { req }) => messages[req.cookies?.lang || "EN"][key];

const registerRules = [
  body("name").optional().isString().withMessage(msg("nameString")).trim(),
  body("email").notEmpty().withMessage(msg("emailRequired")).isEmail().withMessage(msg("emailInvalid")).normalizeEmail(),
  body("password")
    .notEmpty()
  ...
  body("phone").optional().matches(/^\d{3}-\d{3}-\d{4}$/).withMessage(msg("phoneFormat")),
  body("address").optional().isString().withMessage(msg("addressString")).trim(),
  checkValidation,
];
